...
public class SqlMapper {
    ...
    public <T> AList<T> insertMany(Connection conn, List<T> os) {
        ...
        final BeanMetaData beanMetaData = beanRegistry.getMetaData(os.get(0).getClass());
    ...
    private <T> AVector<T> insertManyAutoGenerated(Connection conn, BeanMetaData beanMetaData, List<T> os) {
        return executeUnchecked(() -> {
            ...
            boolean first = true;
            for (T o: os) {
                if (beanRegistry.getMetaData(o.getClass()) != beanMetaData) throw new IllegalArgumentException("multi-insert only for beans of the same type");
    ...
    private <T> AVector<T> insertManyProvidingPk(Connection conn, BeanMetaData beanMetaData, List<T> os) {
        return executeUnchecked(() -> {
            ...
            for (Object withoutPk: os) {
                if (beanRegistry.getMetaData(withoutPk.getClass()) != beanMetaData) throw new IllegalArgumentException("multi-insert only for beans of the same type");
    ...
    public <T> T insert(Connection conn, T o) {
        final BeanMetaData beanMetaData = beanRegistry.getMetaData(o.getClass());
        if (beanMetaData.pkStrategy().isAutoIncrement()) {
    ...
    private <T> T insertProvidingPk(Connection conn, Object withoutPk) {
        return executeUnchecked(() -> {
            final BeanMetaData beanMetaData = beanRegistry.getMetaData(withoutPk.getClass());
    ...
    public boolean update(Connection conn, Object bean) {
        return executeUnchecked(() -> {
            final BeanMetaData beanMetaData = beanRegistry.getMetaData(bean.getClass());
    ...
    public boolean delete(Connection conn, Class<?> beanType, Object pk) {
        final BeanMetaData beanMetaData = beanRegistry.getMetaData(beanType);
