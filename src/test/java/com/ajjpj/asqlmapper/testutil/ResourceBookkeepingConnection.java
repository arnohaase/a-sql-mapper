...
class ResourceBookkeepingConnection implements Connection {

    private final Connection delegate;

    private final AMap<Statement, Throwable> createdStatements = AMutableMapWrapper.empty();

    ResourceBookkeepingConnection(Connection delegate) {
        this.delegate = delegate;
    }

    private <T extends Statement> T registerStatement(T stmnt) {
        createdStatements.put(stmnt, new Throwable("stack trace for Statement creation"));
    ...
    private <T extends Statement> T createStatementWrapper(T stmnt) {
        ...
        T wrapper = (T) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class[]{ proxyClass}, (proxy, method, args) -> {
            try {
                return method.invoke(stmnt, args);
            }
    ...
    void assertResourcesReleased() {
        ...
        if(createdStatements.keySet().exists(s -> !executeUnchecked(s::isClosed))) {
            fail("unclosed statements");
        }
    }
